...
const calendar = {
  ...
  solar2lunar: function (y, m, d) {
    ...
    for (let item of calendar.nStr4) {
      // 公历节日
      if (parseInt(item.substr(0, 2)) == m) {
        if (parseInt(item.substr(2, 4)) == d) {
          // lObj.innerHTML = item.substr(5);
          cFestival = item.substr(5)
        }
      }
      // console.log(item,cFestival,m,d)
    }
    if (m == 5) {
      // 母亲节
      if (fWeek == 0) {
    ...
    for (let item of calendar.nStr5) {
      // 农历节日
      if (parseInt(item.substr(0, 2)) === parseInt(month)) {
        if (parseInt(item.substr(2, 4)) === parseInt(day)) {
          // lObj.innerHTML = item.substr(5);
          lFestival = item.substr(5)
        }
      }
      if (12 == month) {
        // 判断是否为除夕
...
// let html =
//   "<table cellspacing='0'><tr><th>日</th><th>一</th><th>二</th><th>三</th><th>四</th><th>五</th><th>六</th></tr>" // 打印表格第一行(显示星期)
...
//       date = ' ' // 索引小于等于0或者大于月份最大值就用空表格代替
//     } else {
//       date = idx - dayOfWeek + 1
//       data = calendar.solar2lunar(y, m + 1, date)
//     }
//     // console.log(data.cFestival, data.lFestival)
//     html += '<td '
//     if (date == d) {
//       // 高亮显示当天
//       html += 'class="today"'
...
//           ${date != ' ' ? '<div class="td">' : ''}
//           <h3>${date}</h3>
//           <p>${date != ' ' ? data.cFestival || data.lFestival || data.Term || data.IDayCn : ''}</p>
//           ${date != ' ' ? '</div>' : ''}
//           </td>`
//   }
//   html += '</tr>'
// }
// html += '</table>'
// let data = calendar.solar2lunar(y, m + 1, d)
// // console.log(data)
// html += `<h3>公历：${data.cYear}年${data.cMonth}月${data.cDay}日${data.ncWeek}</h3>`
// html += `<h3>农历：${data.IMonthCn} ${data.IDayCn}</h3>`
// html += `<h3>天干地支纪年：${data.gzYear}年 【${data.Animal}年】 ${data.gzMonth}月 ${data.gzDay}日</h3>`
// html += `<h3>${data.astro}</h3>`
// data.isTerm && (html += `<h3>${data.Term}</h3>`)
// data = calendar.solar2lunar(1999, 3, 4)
// html += `<h3>公历：${data.cYear}年${data.cMonth}月${data.cDay}日${data.ncWeek}</h3>`
// html += `<h3>农历：${data.IMonthCn} ${data.IDayCn}</h3>`
// html += `<h3>天干地支纪年：${data.gzYear}年 【${data.Animal}年】 ${data.gzMonth}月 ${data.gzDay}日</h3>`
// html += `<h3>${data.astro}</h3>`
// data = calendar.solar2lunar(1998, 7, 3)
// html += `<h3>公历：${data.cYear}年${data.cMonth}月${data.cDay}日${data.ncWeek}</h3>`
// html += `<h3>农历：${data.IMonthCn} ${data.IDayCn}</h3>`
// html += `<h3>天干地支纪年：${data.gzYear}年 【${data.Animal}年】 ${data.gzMonth}月 ${data.gzDay}日</h3>`
// html += `<h3>${data.astro}</h3>`
// $('.main').html(html)
// console.log(0x001000)

console.log(calendar.solar2lunar(1999, 3, 4))
console.log(calendar.solar2lunar(1998, 7, 3))
